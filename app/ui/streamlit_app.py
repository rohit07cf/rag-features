...
with st.sidebar:
    ...
    try:
        from app.ui.api_client import list_assistants
...
- Model: `{selected["model"]}`
    ...
    st.divider()
    st.page_link("pages/0_Home_Gallery.py", label="🏠 Gallery", icon="🏠")
    st.page_link("pages/1_Create_Assistant.py", label="➕ Create", icon="➕")

    a = st.session_state.get("selected_assistant")
    if a and a.get("type") == "rag":
        st.page_link("pages/2_Upload_Documents.py", label="📄 Upload", icon="📄")
    if a:
        st.page_link("pages/3_Chat_Assistant.py", label="💬 Chat", icon="💬")
...
st.page_link("pages/0_Home_Gallery.py", label="Open Gallery →", icon="🏠")
