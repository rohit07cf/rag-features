...
with st.form("create_assistant_form"):
    ...
    with col1:
        a_type = st.selectbox("Type", ["rag", "model_only"],
    ...
    if a_type == "rag":
        chunk_strategy = st.selectbox(
            ...
            format_func=lambda k: f"{k} — {CHUNK_STRATEGIES[k].split('—')[1].strip()}" if "—" in CHUNK_STRATEGIES[k] else k,
        )
...
if submitted:
    if not name.strip():
        ...
        try:
            from src.ui.api_client import create_assistant
