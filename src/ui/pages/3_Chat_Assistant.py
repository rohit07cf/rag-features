...
if assistant.get("type") == "rag":
    try:
        from src.ui.api_client import get_rag_status

        rag_info = get_rag_status(assistant["id"])
        if not rag_info.get("has_documents"):
            st.warning("This RAG assistant has no documents yet. Upload documents first for best results.")
...
for col, suggestion in zip(suggestion_cols, suggestions):
    if col.button(suggestion[:40] + "..." if len(suggestion) > 40 else suggestion,
                  key=f"sug_{suggestion[:20]}", use_container_width=True):
...
if message:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            try:
                from src.ui.api_client import chat
